...
%% Animation Test - Started 12/7/2012 by Adrian Gose

Beam.Location1 = linspace(1,(Beam.Length-1),20);

for k = 1:20
    Loc1 = Beam.Location1(k);
    a = Loc1;
    Beam.x1 = linspace(0, a, 100); % Gose
    Beam.x2 = linspace(a, L, 100);
    
    Beam.y1 = ((F*b.*x1)/(6*L*E*I)).*(L^2-x1.^2-b^2);                    % 0<x<a
    Beam.y2 = ((F*b)/(6*E*L*I)).*((L/b).*(x2-a).^3+(L^2-b^2).*x2-x2.^3); % a<x<L
    plot(Beam.x1, -Beam.y1, 'b', 'LineWidth',3);
    hold on
    plot(Beam.x2, -Beam.y2, 'b', 'LineWidth',3);
    hold off
